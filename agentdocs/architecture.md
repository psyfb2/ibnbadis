...
| **C** | **Section-slug non-reproducibility.** CSTC-3 hand-made special slugs (`dictionary`, `ending`, `pictionary-and-alphabet`) are not algorithmic. CSTC-4 uses deterministic `section-{1-based-index}-{slug}` (+ optional `/part-{n}`) for both scrape and write-back. | CSTC-4 must key scrape & write-back by the same rule (internally consistent). Cross-ref with CSTC-3 is best-effort by key string only. |
