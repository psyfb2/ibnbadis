...
| **C** | **Section-slug.** CSTC-3 uses hand-made special slugs (e.g. `dictionary`, `ending`, `pictionary-and-alphabet`) that are **not** algorithmically reproducible. CSTC-4 deterministically uses `section-{1-based-index}-{slug}` for **both** scrape and write-back. The key format allows an optional `/part-{n}` segment, but `_current_part_number()` is **currently a stub returning `None`** (pending the §11 live run), so CSTC-4 **emits no `/part-{n}` on any key today**. | CSTC-4 must key scrape and write-back by the *same* deterministic rule; it is internally consistent (no collisions — page numbers are globally sequential, not reset per part). Cross-reference with CSTC-3 keys is **best-effort by key string** only and **will not match CSTC-3 keys containing `/part-`** until part detection is implemented in the live run (§11). |
